	...
	{:else}
		<div class="flex flex-col gap-3">
			<button
				...
				disabled={isLinking}
				class="w-full rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-blue-500/20 transition-all hover:bg-blue-500 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
			>
				{isLinking ? 'Перенаправление...' : 'Войти через YSTU ID'}
			</button>
		</div>
	{/if}
</div>

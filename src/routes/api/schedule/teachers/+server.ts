...
export async function GET() {
	try {
		...
		try {
			const response = await fetchWithTimeout(fetch, `${API_BASE}/actual_teachers`);
			if (!response.ok) {
				if (cachedData) {
					const { timestamp: _t, ...data } = cachedData;
					console.warn(`Teachers API error ${response.status}, using stale cache`);
					return json(data);
				}
				return json(
			...
			if (cachedData) {
				const { timestamp: _t, ...data } = cachedData;
				console.warn('Teachers API unavailable, using stale cache');
				return json(data);
			}
			return json(
				{
		...
		console.warn('Teachers route:', error instanceof Error ? error.message : String(error));
		return json(
			{
				error: 'Internal server error',
				details: error instanceof Error ? error.message : String(error)

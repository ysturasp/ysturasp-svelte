...
<section class="bg-slate-800 rounded-2xl p-6 mt-8">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
      <h3 class="text-xl font-bold text-red-400 mb-2">👎 Худшие предметы</h3>
      <ul class="space-y-4">
        {#if isLoading}
          <li class="skeleton-item">
            <div class="skeleton skeleton-title"></div>
        ...
        {:else}
          {#each antitop10 as item}
            <li class="list-item">
              <div class="text-red-500 font-semibold cursor-pointer" on:click={() => dispatch('viewAgain', { subject: item.subject })}>
                {item.position}. {item.subject}
              </div>
              <div class="text-slate-400">Средний балл: {item.average.toFixed(2)}</div>
              <div class="text-slate-400">Оценок: {item.count}</div>

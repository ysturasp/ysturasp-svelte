<script lang="ts">
	...
	function handleMouseEnter(event: MouseEvent) {
		...
		anime({
			targets: card,
			boxShadow: ['0 0 0 rgba(59, 130, 246, 0)', '0 0 30px rgba(59, 130, 246, 0.3)'],
			scale: 1.02,
			duration: 800,
			easing: 'easeOutElastic(1, .6)'
		});
...
<section
	class="stats-section relative overflow-hidden py-8"
	use:inview={{ unobserveOnEnter: true }}
	on:inview_change={handleInview}
>
	<div class="absolute inset-0"></div>

	<div class="mx-auto grid max-w-5xl grid-cols-1 gap-8 px-4 md:grid-cols-3">
		{#each stats as { value, label, color, icon }}
			<div
				class="stat-card group relative rounded-2xl bg-slate-800 p-8 backdrop-blur-sm"
				style="border: 1px solid {color === 'blue'
					? '#3B82F6'
					: color === 'purple'
						? '#8B5CF6'
						: '#10B981'};"
			...
			>
				<div
					class="particles-container absolute bottom-0 left-0 h-16 w-full overflow-hidden"
				>
					<div class="particles-wave flex {color}-particles"></div>
				</div>

				<div
					class="stat-number text-4xl font-bold"
					style="background: linear-gradient(to right, {color === 'blue'
						? '#60A5FA'
						: color === 'purple'
							? '#A78BFA'
							: '#34D399'}, {color === 'blue'
				...
				<div class="stat-label mt-2 text-slate-400">{label}</div>

				<div
					class="stat-circle absolute -top-2 -right-2 h-4 w-4 rounded-full"
				...
				<svg
					class="absolute top-0 right-0 h-16 w-16 md:h-20 md:w-20"

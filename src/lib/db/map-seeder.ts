...
export async function checkAndSeedMap(pool: Pool) {
	try {
		const checkRes = await pool.query('SELECT COUNT(*) FROM map_sections');
...
async function seedInfrastructure(pool: Pool, TOTAL_FLOORS: number) {
	for (let floor = 1; floor <= TOTAL_FLOORS; floor++) {
